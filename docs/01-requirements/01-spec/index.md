...
> **เลข US ตัวถัดไปที่เปิดได้คือ `US-55`** (แก้ไข 2026-08-23 — ค่าเดิมที่เคยเขียนว่า `US-51` ล้าสมัยแล้ว) — `US-46` ถึง `US-49` ยังถูกจองไว้เป็นข้อเสนอใน [[feature-list-v1|Feature List v1]] §5 ที่ยังรอ Touch/XAVIER ตัดสิน (ไม่เปลี่ยนแปลง) · `US-50` (นมโอ๊ต) ถูกใช้ไปแล้วเมื่อ 2026-08-16 · `US-52` (พร้อมเพย์ QR เคาน์เตอร์) ถูกใช้ไปแล้วเมื่อ 2026-08-16 เช่นกัน แต่เอกสารนี้ไม่เคยถูกอัปเดตตามหลัง PR ที่รวมสองสาย branch เข้าด้วยกัน (2026-08-23) จึงยังค้างค่าเดิม · `US-51` เป็น**ช่องว่างที่ไม่เคยถูกใช้จริง** (ไม่มีเอกสารใดอ้างอิง US-51 เลย ตรวจสอบแล้วเมื่อ 2026-08-23) — **ตั้งใจไม่นำ US-51 กลับมาใช้** เพื่อกันชนกับงานที่อาจค้างอยู่ใน branch อื่นที่ยังไม่ merge เข้ามา ปล่อยเป็นเลขว่างถาวร · `US-53` (แดชบอร์ดวิเคราะห์ยอดขายเชิงลึก, P2/Phase 2) ถูกใช้แล้วเมื่อ 2026-08-23 — ดู [[product-backlog-v1|Product Backlog v1]] หมวด 2.4 · `US-54` (อ่านเรื่องราวเมล็ดกาแฟระหว่างรอออเดอร์ / Bean Story, P2/Phase 2) ถูกใช้แล้วเมื่อ 2026-08-23 เช่นกัน — ดู [[product-backlog-v1|Product Backlog v1]] หมวด 2.7 · **ห้ามใช้เลขซ้ำ**
> ⚠️ **ทราบแล้วว่าตารางทะเบียนด้านบน (แถว `US-xx`, `AC-<US>-<n>`) และรายการเอกสารด้านล่าง (Feature List v1, Traceability Audit v1, Acceptance Criteria v1) ยังอ้างอิง "ครอบ US-01 ถึง US-45"/"46 รายการ" ซึ่งไม่นับ US-50/US-52/US-53 ที่เพิ่มเข้ามาทีหลัง** — เป็นช่องว่างที่เกิดจากหลายรอบแก้ไขที่ไม่ได้ไล่อัปเดตทะเบียนนี้ตาม ไม่ใช่สิ่งที่ตั้งใจแก้ในรอบนี้ (ต้องไล่ตรวจ Feature List v1 §5 และ Acceptance Criteria v1 ใหม่ทั้งชุดถึงจะปิดได้ถูกต้อง) — บันทึกไว้ตรงนี้เพื่อไม่ให้ใครเข้าใจผิดว่าตัวเลขด้านล่างเป็นปัจจุบัน
