...
| รอ/ติดตามสถานะ | เปิดจอดูสถานะเป็นระยะ หรือคุยงานรอไป | หน้าจอติดตามสถานะ | "อีกนานแค่ไหน ไม่อยากเดินไปถาม" | 😐 0 | ไม่มี ETA/คิวที่เหลือ มีแค่สถานะ 3 ระดับ ต้องเปิดจอเองถึงจะเห็นความเปลี่ยนแปลง ไม่มีแจ้งเตือนเชิงรุก | แสดงเวลาโดยประมาณ + แจ้งเตือนเชิงรุก (เสียง/สั่น) เมื่อสถานะเปลี่ยน | US-05 (gap บางส่วน — ดูหัวข้อ gap) |
...
- ไม่มี US รองรับ ETA/จำนวนคิวที่เหลือ ให้ลูกค้ารู้ว่าต้องรออีกนานแค่ไหน (มีแค่สถานะ 3 ระดับ)
- ไม่มี US รองรับการแจ้งเตือนเชิงรุก (เสียง/สั่น) เมื่อสถานะออเดอร์เปลี่ยน — ปัจจุบันเป็นแค่ real-time update ที่ลูกค้าต้องเปิดจอเองถึงจะเห็น
...
journey
    ...
    section สั่งที่เคาน์เตอร์
      ...
      ยืนยันออเดอร์แล้วไม่รู้ว่าต้องไปจ่ายตรงไหน: 2: ต้น
    section จ่ายเงินและรอรับ
      จ่ายเงินที่เคาน์เตอร์ก่อนเสมอ: 4: ต้น, เบียร์
      ยืนรอที่ปลายเคาน์เตอร์โดยไม่มีสัญญาณบอก: 2: ต้น
      ได้รับเครื่องดื่ม: 5: ต้น
      ออกจากร้าน: 4: ต้น
```

> ต่างจาก J1 ตรงที่ **ต้องจ่ายก่อนเสมอ** ตาม Business Rule ข้อ 12 และ **จุดรับของแยกทางกายภาพจากจุดสั่ง** — สองขั้นที่ตกไปที่ 2 คือ moment of truth ทั้งคู่
...
| ยืนยันออเดอร์ | กดยืนยัน → เข้าสถานะ "รอยืนยันการชำระเงิน" | หน้าจอยืนยันออเดอร์ | "ต้องจ่ายตอนนี้เลยใช่ไหม จ่ายที่ไหน" | 😕 -1 (ถ้าไม่ชัดเจน) | **Moment of truth #1** — ถ้าข้อความไม่ชัดว่าต้องไปจ่ายที่เคาน์เตอร์ทันที ลูกค้าจะยืนงงหรือเดินหนีไปเลยเพราะรีบ | ข้อความชัดเจนทันทีหลังยืนยัน "กรุณาชำระเงินที่เคาน์เตอร์" | US-28 (AC ข้อ "เห็นข้อความชัดเจนให้ไปชำระเงิน") |
| จ่ายเงินที่เคาน์เตอร์ | จ่ายเงินสด/บัตรกับแคชเชียร์ | แคชเชียร์ (คนเดียวกับที่ทำเครื่องดื่ม) | "จ่ายเสร็จแล้วใช่ไหม เริ่มทำหรือยัง" | 🙂 +1 | ถ้าแคชเชียร์กำลังยุ่งกับออเดอร์อื่น อาจต้องรอคิวจ่ายเงินก่อนด้วยซ้ำ | จุดจ่ายเงิน/ยืนยันต้องเร็วที่สุดเพราะเป็นคอขวดของทั้ง flow | US-29 |
...
### Moment of truth (J2)
1. **ข้อความหลังกดยืนยันออเดอร์ต้องชัดว่าต้องไปจ่ายเงินที่เคาน์เตอร์ทันที** — ถ้าไม่ชัด ต้นซึ่งรีบมากจะสับสนหรือเดินออกไปโดยไม่จ่ายเงิน กลายเป็นออเดอร์ค้างในระบบ
...
### จุดที่ระบบยังไม่มี US รองรับ (gap)
- ไม่มี US แสดงคิว/ลำดับก่อนหลังของออเดอร์ที่รอจ่ายเงิน หรือประมาณเวลารอที่เคาน์เตอร์
- ไม่มี US รองรับการแจ้งเตือนเชิงรุกเมื่อของเสร็จพร้อมรับที่ปลายเคาน์เตอร์ (ปัจจุบันมีแค่การอัปเดตสถานะแบบ passive)
- ไม่มี US ระบุว่าจะเกิดอะไรขึ้นถ้าลูกค้ายืนอยู่ที่เคาน์เตอร์แต่แคชเชียร์ไม่ทันเห็นออเดอร์ "รอยืนยันการชำระเงิน" (เช่น มัวทำเครื่องดื่มออเดอร์ก่อนหน้าอยู่) — ไม่มี timeout/escalation
...
| 3 | ลูกค้าไม่รู้เวลารอ/สถานะเชิงรุก ทั้ง dine-in และ takeaway — มีแค่สถานะ 3 ระดับ ไม่มี ETA ไม่มีแจ้งเตือนเชิงรุก | J1, J2 | **สูง** | **Gap ใหม่** — เสนอ US-38, US-39 |
...
4. **การแจ้งเตือนเชิงรุก (US-38/US-39 ที่เสนอ) ต้องการลงทุนระดับไหน** — แค่ badge/sound บนหน้าเว็บพอ หรือต้องการถึงขั้น SMS/LINE notification (ซึ่งจะเปิดจุดเก็บข้อมูลส่วนบุคคลใหม่ตาม Business Rule ข้อ 14 ของ [[product-backlog-v1|Product Backlog v1]] และต้องผ่าน PDPA gap analysis ใหม่)
