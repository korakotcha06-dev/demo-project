...
> 🔴 **แก้ไข 2026-08-20 — ย้าย stack ไป VPS ของ TNM + Cloudflare (หัวข้อ 9) และเลือกกลไก auth ของพนักงาน/แอดมิน (หัวข้อ 10) · ตัดสินให้หลายร้านใช้ร่วมกัน (หัวข้อ 11) อ่าน [หัวข้อ 9](#9-คำตัดสินของ-touch--ย้ายจาก-supabase--vercel-ไป-vps-ของ-tnm--cloudflare-2026-08-20) ก่อนอ่านหัวข้อ 3 และ 5**
...
### 5.1 สภาพแวดล้อม  ⚠️ ต้องเขียนใหม่ — ดูหัวข้อ 9.6 · ✅ **เขียนแล้วที่ [[deploy-runbook-v1|Deploy Runbook v1]]**
...
### 5.2 CI/CD และ migration  ⚠️ ต้องเขียนใหม่ — ดูหัวข้อ 9.6 · ✅ **เขียนแล้วที่ [[deploy-runbook-v1|Deploy Runbook v1]] §1 และ §4**
...
### 5.3 โดเมนและ URL scheme ของ QR — สัญญาถาวร (C8) · 🔴 แก้ไข 2026-08-20

**ตัดสินใจก่อนพิมพ์ QR ใบแรก และห้ามเปลี่ยนอีกเลย:**

```
https://<โดเมนของบริการ>/t/{code}   ← QR จุดบริการ (dine-in)
https://<โดเมนของบริการ>/c/{code}   ← QR เคาน์เตอร์ (takeaway)
```

> **สิ่งที่เปลี่ยนจากฉบับแรก:** เดิมเขียนว่า `<โดเมนสั้นของร้าน>` ซึ่งอ่านได้ว่าเป็นโดเมนแยกต่อร้าน · หลังตัดสินในหัวข้อ 11.7 ว่าจะเปิดให้หลายร้านใช้ **ต้องเป็นโดเมนเดียวของบริการสำหรับทุกร้าน** เพราะ QR ที่พิมพ์แล้วแก้ไม่ได้ ถ้าร้านแรกพิมพ์ด้วยโดเมนของตัวเอง ร้านที่สองจะเข้าระบบเดียวกันไม่ได้เลยโดยไม่พิมพ์ใหม่ทั้งร้าน

- `{code}` = สตริงสุ่มสั้น (base32 8-10 ตัว) **ไม่ใช่ id และไม่ใช่หมายเลขจุดบริการ** → เดา URL ของจุดอื่นไม่ได้ และเปลี่ยนชื่อจุดบริการแล้ว QR ไม่พัง (US-42)
- 🔴 **`{code}` ต้องไม่ซ้ำกันทั้งระบบ ไม่ใช่แค่ไม่ซ้ำในร้าน** — บังคับด้วย unique index ระดับตาราง · `{code}` resolve ไปเป็น **(ร้าน, จุดบริการ)** ไม่ใช่ (จุดบริการ) เฉย ๆ · base32 8–10 ตัวมีพื้นที่เหลือเฟือสำหรับหลายร้าน
- **โดเมนต้องเป็นของ TNM และห้ามผูกกับผู้ให้บริการโฮสต์** — ห้ามใช้โดเมนแถมของแพลตฟอร์มใด ๆ บน QR ที่พิมพ์จริง เพราะถ้าย้ายโฮสต์ทีหลังต้องพิมพ์ใหม่ทุกใบทุกร้าน
- ร้านที่อยากมีโดเมนของตัวเองทำได้ **แต่ต้องเป็น redirect ที่ชี้เข้าโดเมนบริการ ไม่ใช่สิ่งที่พิมพ์ลง QR**
...
### 5.4 การสำรองข้อมูลและกู้คืน  ⚠️ ต้องเขียนใหม่ — ดูหัวข้อ 9.6 และความเสี่ยง R14 · ✅ **รายการที่ต้องทำอยู่ที่ [[deploy-runbook-v1|Deploy Runbook v1]] §3.2** (ยังไม่ได้ลงมือ — ทำก่อนเปิดร้าน)
...
## 11. เปิดให้ร้านอื่นมาใช้ด้วย — ✅ ตัดสินแล้วว่าเอา (2026-08-20)

> หัวข้อนี้เริ่มจากเป็นคำถามเปิด และ**ถูกตอบแล้วใน 11.7** — อ่าน 11.7 ก่อนถ้าต้องการแค่ข้อสรุป
...
### 11.7 ✅ ตอบแล้ว — เอา (2026-08-20)

Touch ยืนยันเส้นทางจริง: **ทำใช้เองก่อน → ร้านคาเฟ่ของญาติ → ร้านของเพื่อน ๆ ที่พร้อมทดลอง → ถ้าดีจริงจะกระจายเอง** โดยตั้งเป้าตลาด **เชียงใหม่ · เชียงราย · ลำพูน**

หัวข้อ 11 จึงไม่ใช่คำถามเปิดอีกต่อไป **11.2 และ 11.3 ต้องทำ ไม่ใช่แค่เผื่อไว้** และ **11.6 มีผลทันที — เก็บ RLS ไว้**

#### ต้องทำก่อนพิมพ์ QR ใบแรกของร้านญาติ

QR ของร้านแรกคือสิ่งที่ล็อกโครงสร้างของทุกร้านที่ตามมา ถ้าใบแรกพิมพ์ผิดแบบ ร้านที่สองจะเข้าระบบไม่ได้โดยไม่พิมพ์ใหม่ทั้งร้าน

1. **โดเมนเดียวสำหรับทุกร้าน** ตาม 11.2 — ห้ามใช้โดเมนต่อร้านบน QR ที่พิมพ์
2. **`{code}` ไม่ซ้ำทั้งระบบ** และ resolve เป็น (ร้าน, โต๊ะ)
3. **`shop_id` ใน schema** ตาม 11.3 ตั้งแต่ migration แรก ตอนที่ยังไม่มีข้อมูลจริงสักแถว

#### สิ่งที่ยัง **ไม่** ต้องทำ แม้จะตอบว่าเอาแล้ว

การเก็บเงินรายเดือน · หน้าเปิดร้านใหม่แบบบริการตัวเอง · หน้าแอดมินระดับผู้ให้บริการ · แพ็กเกจ

**ร้าน 5–10 ร้านแรกเปิดให้ด้วยมือถูกกว่าสร้างระบบเปิดเอง** และการเปิดให้ด้วยมือยังบังคับให้เราได้คุยกับเจ้าของร้านทุกรายในช่วงที่ยังต้องเรียนรู้ว่าเขาติดตรงไหน ซึ่งเป็นข้อมูลที่มีค่ากว่าระบบอัตโนมัติในช่วงนี้

#### ความเสี่ยงที่เปลี่ยนน้ำหนักทันที

- 🔴 **R15 (VPS เครื่องเดียว) ต้องมีแผนก่อนร้านที่ 3 ไม่ใช่ก่อนเปิดร้านแรก** — ร้านเดียวล่มคือเรื่องของเรากับญาติ · สามร้านล่มพร้อมกันคือชื่อเสียงที่ใช้ขยายต่อไม่ได้อีก
- 🔴 **R14 (backup) เปลี่ยนจากวินัยเป็นความรับผิด** — ข้อมูลยอดขายของร้านคนอื่นอยู่ในมือเรา
- **ใหม่: ข้อมูลรั่วข้ามร้าน** ตาม 11.5 — ร้านญาติกับร้านเพื่อนอยู่ฐานเดียวกัน คิวรีที่ลืม `shop_id` หนึ่งบรรทัดคือการให้ร้านหนึ่งเห็นยอดขายอีกร้าน · **ทดสอบด้วยร้านเดียวจะผ่านเสมอ** ต้องมีเคสทดสอบที่ใช้สองร้านตั้งแต่ต้น

#### ทำไมเส้นทางนี้ถึงเป็นเส้นที่มีโอกาสชนะจริง

ไม่ได้ชนะด้วยการมีฟีเจอร์มากกว่า — ชนะเพราะ**เลือกสนามที่คู่แข่งลงมาเล่นด้วยยาก** คู่แข่งรายใหญ่เกิดที่กรุงเทพฯ ขายทั่วประเทศและต่างประเทศผ่านช่องทางระยะไกล ส่วนเราเริ่มจากคนที่รู้จักกันจริงในพื้นที่ ไปติดตั้งเองได้ และเข้าใจร้านแบบภาคเหนือ

ข้อแลกคือ **ร้านกลุ่มนี้ไม่ใช่ร้านที่มีทีมไอที** ระบบต้องทำงานได้โดยเจ้าของร้านไม่ต้องเข้าใจอะไรเลย ซึ่งตรงกับ C5 และ persona P5 ที่เขียนไว้แล้วตั้งแต่ต้น — ข้อจำกัดเดิมกลายเป็นจุดแข็งของกลยุทธ์นี้พอดี

รายละเอียดการเทียบกับคู่แข่งอยู่ที่ [[../../01-requirements/01-spec/competitor-papaya-v1|Competitor Reference — Papaya v1]]

---

## 12. MCP — ลูกค้าคนแรกของมันคือเราเอง ไม่ใช่เจ้าของร้าน (2026-08-20)

คู่แข่งขาย Open API และ MCP ไว้ที่แพ็กบนสุด ฿5,000 ต่อสาขาต่อเดือน · **TNM ทำ MCP ใช้งานจริงอยู่แล้วที่ TNM Console** จึงไม่ใช่ความสามารถที่ต้องไปเรียนรู้ใหม่ แต่ **มุมที่คุ้มที่สุดไม่ใช่มุมที่เขาขาย**

### 12.1 มุมที่เขาขาย และทำไมมันอ่อนกว่าที่คิดสำหรับตลาดของเรา

เขาขาย MCP เป็นของสำหรับ**เจ้าของร้าน** — ถามยอดขายด้วยภาษาคน แทนการเรียนรู้หน้ารายงาน

ฟังดูตรงกับ C5 (เจ้าของร้านไม่สายเทค) มาก **แต่มันต้องการให้เจ้าของร้านมี AI client ของตัวเองก่อน** ซึ่งเป็นข้อสมมติที่ไม่จริงกับกลุ่มร้านที่เราจะไปหาในเชียงใหม่ เชียงราย ลำพูน · ถ้าจะให้เจ้าของร้านได้ประโยชน์จริงต้องเป็น **ผู้ช่วยที่ฝังอยู่ในหน้าแอดมินของเราเอง** ไม่ใช่ MCP ที่ต้องต่อจากข้างนอก — และนั่นเป็นงาน Phase 1 ขึ้นไป ไม่ใช่ Phase 0

### 12.2 🔴 มุมที่คุ้มจริง — ใช้ดูแลหลายร้านพร้อมกัน

R13 (ภาระ ops ย้ายมาอยู่ที่ TNM) ถูกประเมินไว้ว่า **โอกาสสูง ผลกระทบสูง** และ 11.7 ระบุว่าต้องมีแผนก่อนร้านที่ 3

**MCP คือมาตรการของ R13 โดยตรง** — พอมี 5–10 ร้านอยู่บนเครื่องเดียวกัน คำถามที่ต้องตอบทุกวันคือ

- ร้านไหน backup ล่าสุดเมื่อไหร่ และ restore ผ่านหรือเปล่า
- ร้านไหนมีออเดอร์ค้างในสถานะเดิมนานผิดปกติ
- ร้านไหน realtime หลุดไปอยู่โหมด polling
- เมื่อคืนมีบิลไหนปิดด้วยยอดที่ไม่ตรงกับที่แคชเชียร์เห็นบ้าง

ถ้าต้องเปิดหน้าจอไล่ดูทีละร้าน งานนี้จะไม่ถูกทำหลังร้านที่สาม · ถ้าถามได้ทีเดียวข้ามทุกร้าน มันจะถูกทำทุกวัน **นี่คือความต่างระหว่างมาตรการที่เขียนไว้ในเอกสารกับมาตรการที่เกิดขึ้นจริง**

### 12.3 รูปแบบที่พิสูจน์แล้วจาก TNM Console ที่ควรยกมาทั้งชุด

- **อ่านอย่างเดียวเป็นค่าเริ่มต้น** — เครื่องมือส่วนใหญ่ต้องอ่านได้อย่างเดียว ไม่ใช่แก้ข้อมูลได้
- **ปิดบังข้อมูลส่วนบุคคลตามสิทธิ์ของผู้เรียก** ไม่ใช่ตามความตั้งใจของผู้เขียนเครื่องมือ
- **สิ่งที่เขียนข้อมูลต้องเป็นคู่ preview แล้วค่อย confirm** ไม่ใช่คำสั่งเดียวจบ — เป็นรูปแบบเดียวกับ maker-checker และเข้ากับ BR 5 ที่บังคับให้การยกเลิกต้องมีเหตุผลเสมอ
- **ขอบเขตต้องผูกกับ `shop_id` เสมอ** ตาม §9 ของ [[data-model-v1|Data Model v1]] — เครื่องมือที่ลืมผูกขอบเขตคือรูรั่วข้ามร้านชนิดเดียวกับคิวรีที่ลืมเงื่อนไข

### 12.4 สิ่งที่ยกมาใช้ตรง ๆ ไม่ได้

TNM Console เป็น Laravel ส่วนระบบนี้เป็น Next.js — **สูตรที่มีอยู่เป็นสูตรของ Laravel ยกมาทั้งดุ้นไม่ได้** สิ่งที่ยกมาได้คือ *รูปแบบ* ตาม 12.3 ไม่ใช่โค้ด · ต้องทำ MCP server ฝั่ง Node ใหม่ ซึ่งเป็นงานที่ไม่ใหญ่แต่ต้องนับว่าเป็นงาน

### 12.5 สรุปสำหรับการวางแผน

| ใคร | ได้อะไร | เมื่อไหร่ |
|---|---|---|
| **TNM (ops)** | ดูแลหลายร้านได้จริงโดยไม่ต้องไล่เปิดทีละร้าน — มาตรการของ R13 | **ก่อนร้านที่ 3** |
| เจ้าของร้าน | ผู้ช่วยฝังในหน้าแอดมิน ไม่ใช่ MCP ที่ต้องต่อเอง | Phase 1 ขึ้นไป |
| ลูกค้าที่มี AI client เอง | ต่อ MCP เข้าระบบตัวเอง | ทีหลัง ถ้ามีคนขอ |

**ไม่ต้องทำใน Phase 0** — Phase 0 มีร้านเดียว เปิดหน้าจอดูเองได้ · แต่ควรออกแบบ Route Handler ตั้งแต่ต้นโดยรู้ว่าจะมีชั้นนี้มาเกาะทีหลัง ซึ่งไม่ต้องทำอะไรเป็นพิเศษนอกจากอย่าฝังตรรกะธุรกิจไว้ในหน้าจอ
