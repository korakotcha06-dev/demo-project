...
> 🔴 **เพิ่ม 2026-08-28 (sync จากโค้ดจริง) — `ready → completed` ที่ออกแบบไว้ข้างบนตั้งแต่ต้นใช้งาน
> ได้จริงแล้วทั้งสองช่องทาง** `qr-order-app@84b094e` บน branch `feat/sales-analytics` (ยังไม่ merge
> `main`) — trigger ของ DB (migration 007) อนุญาต transition นี้ไว้ตั้งแต่ต้นจริง แต่**ไม่มีจุดใดในแอป
> เรียกใช้เลย**: จอสถานี S-02 มีแค่ปุ่มถอยกลับ "กำลังทำ" บนตั๋ว `ready` ไม่มีปุ่มไปข้างหน้า ผลคือ
> ช่องทางเคาน์เตอร์ไม่มีทางถึง `completed` เลยแม้แต่ทางเดียว (ไม่มีขั้นปิดบิลรวมแบบโต๊ะ) ส่วนช่องทาง
> โต๊ะไปถึงได้ทางเดียวคือ auto ตอนปิดบิลทั้งรอบ (บรรทัด `ready --> completed: ... table=auto ตอนปิดบิล`
> ข้างบนยังถูกต้องอยู่) ซึ่งอาจไกลจากตอนเสิร์ฟจริงมาก — ตอนนี้เพิ่มปุ่มบนตั๋ว `ready` ให้กดตรงไป
> `completed` ได้ทั้งสองช่องทาง (ข้อความปุ่มต่างกัน: โต๊ะ = "เสิร์ฟแล้ว" · เคาน์เตอร์ = "รับของแล้ว")
> **ช่องทางโต๊ะจึงมีสองทางไป `completed` พร้อมกันตอนนี้** — กดตรงที่ตั๋ว (ใหม่) กับ auto ตอนปิดบิล
> (เดิม) — ทั้งคู่ยังอยู่ใต้กฎเดิมของ diagram ข้างบนเพราะออกจาก `ready` เหมือนกัน ไม่มี transition ใหม่
> ที่ไม่เคยอนุญาตไว้ · ฝั่งลูกค้า (`order-views.ts`) เพิ่มการซ่อนออเดอร์ `completed` ของช่องทางเคาน์เตอร์
> ที่เกิน 3 นาทีด้วย (ค่าคงที่ตั้งชื่อจุดเดียว ยังไม่ใช่คำตัดสินปิดประเด็น) — รายละเอียดเต็มอยู่ที่
> [[../01-prototypes/screen-inventory-v1|Screen Inventory v1]] หัวข้อ S-02 และ
> [[../../05-log/changelog|changelog]] วันที่ 2026-08-28
