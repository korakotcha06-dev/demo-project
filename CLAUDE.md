...
- 🔴 **หลัง pull ต้องรัน `pnpm migrate`** — โปรเจกต์นี้เพิ่ม migration บ่อย
  (ถึง `023` เมื่อ 2026-08-23) · ไม่รันแล้วแอปจะพังตอนเปิดหน้าที่ใช้ตารางใหม่

**ทั้งสองรีโป `main` เป็นล่าสุดแล้ว** — งานของวันที่ 2026-08-23 merge ครบทั้งสองฝั่ง
(โค้ด PR #1 · เอกสาร PR #13)

> 📌 **จงใจไม่เขียนชื่อ branch ที่กำลังทำงานอยู่ไว้ที่นี่** — เขียนแล้วมัน**ผิดทันที
> ที่ merge** และวันนี้ผิดไปสองรอบภายในไม่กี่นาที · อยากรู้ว่ามีอะไรค้างให้ดูที่
> `git status` กับ PR ที่เปิดอยู่ ซึ่งเป็นแหล่งที่บอกความจริงเองโดยไม่ต้องมีใครมาอัปเดต
...
🔴 **นี่ไม่ใช่ build tool** — vault นี้ยังไม่มี `package.json` ตามเดิม · สคริปต์นี้บังคับกฎ
สองข้อที่เขียนไว้แล้วแต่ไม่เคยมีใครบังคับ จึงพังเงียบ ๆ ได้ตลอด:

1. **wikilink ต้องไม่ขาด** — Obsidian แสดงลิงก์เสียเป็นสีจาง ไม่ใช่ error คนเขียนไม่รู้ตัว
2. **`qr-order-app@<sha>` ต้องชี้ไป commit ที่มีอยู่จริง** — ผลพวงของการแยกสองรีโป
   (ดู decision log 2026-08-23) · เลข sha พิมพ์มือล้วน rebase ทีเดียวก็ตายเงียบ

หา path ของรีโปโค้ดจากบรรทัด `local:` ข้างบนนี้เอง (ทับด้วย env `QR_APP_REPO` ได้) ·
ถ้าไม่มีรีโปโค้ดในเครื่องจะ**ข้ามด่านที่ 2 ไม่ใช่ fail** เพื่อให้คนที่ clone แค่ vault รันได้
